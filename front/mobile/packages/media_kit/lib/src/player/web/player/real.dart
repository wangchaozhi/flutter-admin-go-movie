...
class WebPlayer extends PlatformPlayer {
  ...
  Future<void> setAudioTrack(
    ...
    Future<void> function() async {
      ...
      // FORK: HLS audio-rendition switching. The track id is the rendition
      // index ('auto' / '0' selects the default rendition). Two playback paths
      // exist on web and neither is served by the original <source> hack:
      //   - hls.js (Chrome/Firefox): set hls.audioTrack.
      //   - native HLS (Safari/iOS): toggle element.audioTracks[i].enabled.
      // Never throw for a non-uri track — just no-op if switching is
      // unavailable. See packages/media_kit/README_FORK.md.
      if (!track.uri) {
        // FORK: switch the HLS audio rendition. The track id is the rendition
        // index ('auto'/'no' select the default, index 0). Any interop failure
        // (hls.js not ready, or a browser without the AudioTrackList API) must
        // degrade to a no-op — never break the caller, which on init would
        // surface as a fatal player error.
        var index = 0;
        if (track.id != 'auto' && track.id != 'no') {
          index = int.tryParse(track.id) ?? 0;
        }
        final hls = _hls;
        if (hls != null) {
          // hls.js (Chrome/Firefox/etc.) validates the id itself, ignoring
          // out-of-range values and calls made before the manifest is parsed.
          try {
            hls.audioTrack = index;
          } catch (_) {}
        } else {
          // Native HLS (Safari/iOS): toggle the HTML AudioTrackList entries.
          // Some browsers do not implement element.audioTracks at all —
          // accessing it throws, so it must be isolated.
          try {
            final list = element.audioTracks;
  ...
  void _loadSource(Media media) {
    try {
      if (_isHLS(media.uri)) {
        ...
        // FORK: retain the instance so [setAudioTrack] can switch HLS audio.
        _hls = hls;
      } else {
        // FORK: non-HLS media has no hls.js instance to drive.
        _hls = null;
        // Default
  ...
  bool _isHLS(String src) {
    // FORK: Prefer hls.js whenever it is supported, so HLS alternate-audio
    // switching works via [Hls.audioTrack]. The upstream heuristic preferred
    // native HLS whenever canPlayType() was non-empty, but Chromium browsers
    // report "maybe" for HLS while NOT implementing video.audioTracks — leaving
    // no way to switch the audio track. hls.js (MSE) is the standard, reliable
    // HLS path on Chrome/Edge/Firefox/Android and on iPad/macOS Safari.
    //
    // Where hls.js is unavailable (MSE missing — notably iPhone Safari),
    // isHLSSupported() is false and we fall back to native HLS, which Safari
    // plays well and whose video.audioTracks API works for audio switching.
    if (src.toLowerCase().contains('m3u8') && isHLSSupported()) {
      return true;
    }
    return false;
  }
  ...
  /// FORK: active [Hls] instance for the current media (HLS playback only).
  /// Used by [setAudioTrack] to drive hls.js alternate-audio switching.
  /// See packages/media_kit/README_FORK.md.
  Hls? _hls;
