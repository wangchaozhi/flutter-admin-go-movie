...
extension ExtensionHls on Hls {
  ...
  // --------------------------------------------------
  // FORK: HLS alternate-audio switching support.
  // Upstream media_kit does not drive hls.js for audio-track selection on web.
  // These bindings let [Player.setAudioTrack] switch the active HLS audio
  // rendition. See packages/media_kit/README_FORK.md.

  /// Selects the audio rendition by its hls.js id (the index of the
  /// `#EXT-X-MEDIA:TYPE=AUDIO` rendition in the master playlist). hls.js
  /// validates the id and ignores out-of-range / pre-manifest values.
  external set audioTrack(int value);
}
